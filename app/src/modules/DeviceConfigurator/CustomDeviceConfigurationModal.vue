...
import { getModuleDefinition } from "@src/config/moduleDefinition";
...
function getImage(moduleName: string): string {
    ...
    <v-card>
      <v-container>
        ...
        <div class="config-device">
          <div
            class="config-device-row"
            :class="['device-width-' + modulesWidth]"
          >
            <div
              v-for="(num, index) in numberOfModules"
              class="config-module"
              @click="() => showModuleSelector(index)"
            >
              <img :src="getImage(<string>selectedModules[index])" alt="" />
            </div>
          </div>
        </div>
      </v-container>
      ...
      <v-card-actions>
        <v-btn color="primary" variant="outlined" @click="applyConfiguration"
          >Apply configuration
        </v-btn>
